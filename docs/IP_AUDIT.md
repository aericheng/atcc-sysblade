...
| (!) **未確認** | TRI 公開時的 **specific license**(可能是 CC-BY-NC,可能是 CC-BY,可能是 custom academic license)。**ATCC 競賽是學術 / 教育用途**,絕大部分 license 都允許,但若日後商業化必須回頭查 |
...
| 風險 | (綠) 低 — **datasheet 規格屬廠商主動公開資訊**,引用作為產品比較 / 技術參考屬合理使用 |
...
| 提及 | 位置 | wording | 風險評估 |
|---|---|---|---|
| **「GB200」 / 「GB300」 / 「NVL72」** | repo 廣泛 | 「為 GB200 NVL72 機房設計」/「GB200 ±30 % dV/dt」/「one GB200 NVL72 rack pulls 120 kW」 | (綠) **descriptive 使用 NVIDIA 產品名稱指涉客戶端機房,nominative fair use 標準案例**。**沒有暗示 NVIDIA 背書**,沒有用 NVIDIA logo,沒有用 trademark symbol 偽裝官方合作 |
| **「Bluefield BMC」 / 「nvml」 / 「IPMI」 / 「Redfish」** | `whitepaper.md` §2.1.1 D | 「Power-cap API 候選:NVIDIA nvml、IPMI Power Capping Spec 1.0、Redfish PowerLimit resource」 | (綠) **descriptive API 名稱列舉**,我們明文標「W3 EVT 階段實測後再 commit」— **沒有實作對 nvml 的依賴** |
| **NVIDIA arXiv 論文** | `whitepaper.md` 第九章 [10] / `apps/web/src/app/page.tsx` line 202 | 「Choukse, E., Buck, I., Alben, J. et al. (Microsoft + NVIDIA, 2025), arXiv:2508.14318」 | (綠) **學術引用完整** + 我們所有衍生數字明標「團隊依本文 GB200 power-swing 分析推導」**不是引用其結論**,是 derived 推導 |
| **「NVIDIA put capacitors inside the GB300 PSU; we put a [hybrid module]」** | `apps/web/src/app/page.tsx` line 202-204 | 對 NVIDIA GB300 PSU 內建電容架構的 **descriptive 比較陳述** | (黃) **要顧問判讀**:此句陳述屬「NVIDIA 已公開的 GB300 架構特徵」(NVIDIA 在 OCP / Open Hardware 會議 publicly disclosed)。**若 NVIDIA 沒公開 capacitor placement 細節**,此句可能成為「unverified competitive claim」需要 cite 來源或軟化 wording |
| **「Neural-ART NPU」** | `whitepaper.md` 附錄 C | **這是 STMicroelectronics 的商標,不是 NVIDIA**。Neural-ART 是 STM32N6 的 NPU 名稱 | (綠) ST 商標,descriptive 使用 |

**對 NVIDIA 的 IP 立場明確**:
- [x] 沒用 CUDA / cuDNN / TensorRT / NVIDIA Triton 等 proprietary SDK
- [x] 沒實作任何 NVIDIA-licensed code
- [x] 沒用 NVIDIA 商標 logo / 顏色 / 暗示官方合作
- [v] 所有 NVIDIA 名詞都是「客戶端產品」descriptive 引用(用戶要 BBU,客戶用的是 GB200,我們講「為 GB200 設計」屬正常技術行銷語言)
...
**風險評估**:(綠) 全部屬 nominative fair use。**§3.4 競品比較表的 wording 要顧問細看**
...
| 陳述 | 位置 | 我們的根據 | 風險 |
|---|---|---|---|
| 「Eaton 賣 LIC 模組,但**客戶要自己整合控制律**」 | `whitepaper.md` §1.2 | Eaton XLR product page 確實只賣模組,沒提供整合 firmware | (綠) 事實陳述 |
| 「Vertiv 等只賣 48 V,客戶 2027 後須 forklift 換代」 | 廣泛 | 我們的 HVDC 換代論述,「2027 後 forklift」是行業共識 | (黃) 「forklift」用語可能 over-state,可改「需重新採購 / 替換」 |
| 「Eaton 2024 全球營收 USD 24.9 B / Vertiv USD 8.0 B / Schneider EUR 38.2 B」 | `whitepaper.md` §3.4 / `INVESTOR_BRIEF.md` | 全部來自 publicly disclosed annual reports / financial filings | (綠) 公開財報 |
| 「~ 15 %(Eaton 北美機房 BBU 市占)」 | `whitepaper.md` §3.4 | 「產業分析師估算」 — 我們明文標「BBU 細分項各廠商不公開,用以表達規模量級而非精確數字」 | (黃) 已加 disclaimer,但律師可建議連這個都改為「估算範圍 10-20%」之類 |
| 「策略上看不上小規模 BBU」(對 Vertiv 的策略推論) | `whitepaper.md` §3.4 | 我們的 strategic-moat 推論,**不是引用 Vertiv 官方聲明** | (黃) 「看不上」用語建議改 「未公開 prioritize 此 segment」 |
...
| 名詞 | 我們的承諾 | 實際 repo 內 |
|---|---|---|
| **車規 LFP 高功率 cell(LG ESS B-series / Samsung SDI 高功率版)** | EVT 階段選型 | [x] 沒採購、沒測試、沒寫進 BoM。僅是「候選方向」字串提及 |
| **NVIDIA nvml / IPMI Power Capping / Redfish PowerLimit** | EVT 階段 GPU power-cap API 選型實測 | [x] 沒實作對任何 API 的綁定。僅是「候選 API」字串 |
| **OCP ORV3 12U 機構** | EVT 階段機構設計 | [x] 沒機構檔案 / 沒 CAD。僅是「將對齊 ORV3」承諾 |
| **UL 1973 / NFPA 855 / OCP 認證** | 2027 Q3 認證 | [x] 沒送測。文件多處明文「W3+ 路線圖,複賽範圍外」 |

**為什麼這節重要**:律師可能擔心「你們有寫 nvml 是不是意味著有用 NVIDIA 的 SDK」 — 沒有。**全部都是 placeholder 字串,沒任何 import / link / 二進位嵌入**。

---

## (綠) 不在本 audit 範圍(用戶指示先撇除)
...
**本 repo 的 IP 立場**:
- [v] **沒有實作任何 NVIDIA / Eaton / Maxwell / JK-BMS 廠商 IP**(原 v1.x JK-BMS protocol parser 已隨硬體退貨於 2026-05-27 刪除)
- [v] **所有開源依賴 license 合規**,使用方式符合各自 license 條款
- [v] **所有學術引用 attribution 完整**
- (黃) **商業比較主張 wording 可進一步保守化**(顧問建議下調整)
- (黃) **Severson dataset 學術 → commercial 過渡時要回去確認 license**

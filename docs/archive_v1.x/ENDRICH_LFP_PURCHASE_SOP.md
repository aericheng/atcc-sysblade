...
> **狀態:** 規格已驗證 [v] / 通路已驗證 [v] / **電話確認 + 下單** [ ] (你現在要做的)
...
| 規格 | 數值 | 我們需求 | OK? |
|---|---|---|---|
| 化學系統 | LiFePO4 Nanophosphate® | 真 LFP | [v] |
| 標稱電壓 | 3.3V | 3.2-3.3V | [v] |
| 充飽電壓 | 3.65V | 3.65V(LFP 絕對不能 4.2V) | [v] |
| 放電截止 | 2.0V | 2.0V | [v] |
| 容量 | 2.5 Ah | 原規劃 5 Ah → **減半**(見下方說明) | (!) |
| 連續放電 | 50A(20C) | 我們連續 12.5A,**4× 餘量** | [v] |
| Peak 放電 | 120A(48C, 10s pulse) | 我們 M3 peak 25A,**4.8× 餘量** | [v] |
| 內阻 | ~6 mΩ | Power Cell 級,發熱低 | [v] |
| 循環壽命 | 1,000+ @ 100% DOD | 充足 | [v] |
...
電話:[vendor phone]      ← 今天打這支
傳真:[vendor fax]
Email:[vendor email](業務 [vendor contact])
...
> 撥 **[vendor phone]**,找 **[vendor contact] 業務**。
> 如果[vendor contact]不在,改問:「請問誰能接 A123 / Lithium Werks ANR26650M1B 樣品單?」
...
| **B. 介於 bulk/零售** | NT$ 250 × 12 | NT$ 3,000 | Buffer 不變(8,066)| [v] 立刻下單 |
| **C. 只給零售** | NT$ 335 × 12 | NT$ 4,020 | Buffer **−1,020**(7,046,警戒線 5,000 之上)| [v] 仍可下單(借 PSU+萬用表的版本 Buffer 還有 7,046) |
| **D. 零售 + 不借設備** | NT$ 335 × 12 | NT$ 4,020 | Buffer **4,746**(跌破警戒線 5,000 (!))| (!) 改買 **10 顆**(8 主用 + 2 備品)= NT$ 3,350,Buffer 拉回 5,416 |
...
- 預算 break-even
- 時程:**7-10 天**(可能 miss W1 5/22 (!))— 只在 Endrich 完全拒絕時才用
...
5. **填回 PURCHASE_LIST.md** — 「[v] 到貨 5/21,實付 NT$ ___,單顆 OPV 範圍 ___ V-___ V」
6. **通知隊長** — Slack / Discord 一句「LFP 到貨完成,可進 §4.2.1 first-charge SOP」

(!) **絕對不要做:**
- [x] 不要拆 shrink wrap 後馬上焊接 — 先 first-charge balance(見 `docs/BBU_IMPLEMENTATION_PLAN.md §4.2.1`)
- [x] 不要把 cell 隨便丟桌上疊 — 短路風險,放電池盒或單獨絕緣袋
- [x] 不要用 Li-ion 充電器充 — 4.2V 會炸,只能用 LFP 專用充電器或 JK-BMS 控充
...
| 來源 | LFP 真假驗證 | 結論 |
|---|---|---|
| 蝦皮「Panasonic-5000」 | [x] 充飽 4.2V = Li-ion | 拒絕 |
| Alibaba「Factory Wholesale 26650 LFP」 | [x] 照片印 Li-ion 3.7V | 拒絕 |
| AGM-ICR 26650 | [x] ICR prefix = LiCoO₂,充飽 4.2V | 拒絕 |
| lithium-cell.com 26650 5000mAh 10A | [x] WebFetch 證實 3.7V Li-ion(BSMI 合格的 Taiwan 賣家也標錯) | 拒絕 |
| **Endrich Lithium Werks ANR26650M1B** | [v] 原廠 Nanophosphate® LFP / datasheet 全網對齊 | **可下** |
...
| # | 連結 | 用途 | 狀態 |
|---|---|---|---|
| 1 | <https://www.endrich.com.tw/en-us/index> | Endrich 官網(身分驗證:Taiwan Lithium Werks 經銷商) | [v] 200 OK |
| 2 | <https://lithiumwerks.com/products/lithium-ion-26650-cells/> | Lithium Werks 原廠產品頁(規格證據) | [v] 200 OK |
| 3 | <https://www.batteryspace.com/prod-specs/6610.pdf> | A123 ANR26650M1-B datasheet PDF(2.9 MB,規格證據) | [v] 200 OK |
| 4 | <https://www.nkon.nl/en/a123-systems-anr26650m1b-a-grade-3-3v-a-grade.html> | nkon.nl 荷蘭備案(Plan B2 用)| (!) WebFetch 被 bot 擋(403),真人瀏覽器正常 |
| ~~5~~ | ~~tw.buya123products.com B2C 子站~~ | 移除 — 站點 down(ECONNREFUSED) | [x] 已 down |

---

## ⑩ 完成 checklist

- [ ] 打電話 [vendor phone](找 [vendor contact])
- [ ] 確認:bulk 價 / 庫存 / 同批號 / 5/21 到貨 / datasheet PDF / 二聯式發票 / 付款方式
- [ ] 依「情境 A/B/C/D」決定 12 顆 or 10 顆
- [ ] 當天匯款 / ATM 完成
- [ ] 把收件單號 + 預計到貨時間貼回隊伍 channel
- [ ] 5/21 收貨 → 執行 §⑥ 收貨 SOP
- [ ] 把實付金額更新到 `docs/archive_v1.x/PURCHASE_LIST.md` §1A LFP 26650 行
